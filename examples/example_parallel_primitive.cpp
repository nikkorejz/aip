...
int main() {
    ...
    auto evaluate = [&](std::size_t global) -> EvalResult {
        auto pm = orch.makePiecewise(global);
        ...
        const double r = pearsonCorrelation(model_data, original);
        return {r, global};
    };
    ...
    std::mutex ioMutex;  // чтобы аккуратно печатать прогресс (опционально)

    auto results = aip::search::parallelForIndicesAsync(
        0, total, [&](std::size_t global) { return evaluate(global); }, std::thread::hardware_concurrency(),
        [&](std::size_t d, std::size_t tot) {
            done.store(d, std::memory_order_relaxed);

            // печать прогресса раз в ~1%
            if (tot >= 100) {
                const std::size_t step = tot / 100;
                if (step > 0 && (d % step) == 0) {
                    std::lock_guard<std::mutex> lock(ioMutex);
                    std::cout << "\rProgress: " << (100 * d / tot) << "% (" << d << "/" << tot << ")" << std::flush;
                }
            }
        });
    ...
    double bestR = __DBL_MIN__;
    std::size_t bestGlobal = 0;
    for (const auto& r : results) {
        if (std::isfinite(r.r) && r.r > bestR) {
            bestR = r.r;
            bestGlobal = r.global;
        }
    }
    ...
    std::cout << "  pearson r: " << std::setprecision(6) << bestR << "\n";
    std::cout << "  best global index: " << bestGlobal << "\n\n";
